...
- **テストで使用するデータはシナリオ内で自ら作成すること** — 外部で事前にデータが存在することを前提としないでください。`executeDb` でのSQL実行やUI操作を通じて、テストに必要なデータをシナリオ内で準備してください

### 7.2 シナリオ構成の推奨パターン

1. **セットアップシナリオ**（DB使用時）: テーブル作成、データ初期化、必要に応じてテストデータの投入（`executeDb` で INSERT）
2. **操作テストシナリオ**: 画面遷移 → 操作 → 検証。テストに必要なデータは、セットアップシナリオでの事前投入またはシナリオ内のUI操作・`executeDb` で作成する
...
- [ ] テストで使用するデータがシナリオ内で作成されている（外部の事前データに依存していない）
